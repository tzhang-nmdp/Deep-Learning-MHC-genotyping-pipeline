...
from kirai_pytorch.utils import get_msa_embedd, get_esm_embedd, get_prottran_embedd, exists
from kirai_pytorch.constants import MSA_MODEL_PATH, MSA_EMBED_DIM, ESM_MODEL_PATH, ESM_EMBED_DIM, PROTTRAN_EMBED_DIM

from einops import rearrange

class ProtTranEmbedWrapper(nn.Module):
    def __init__(self, *, kirai):
        super().__init__()
        from transformers import AutoTokenizer, AutoModel

        self.kirai = kirai
        self.project_embed = nn.Linear(PROTTRAN_EMBED_DIM, kirai.dim)
    ...
    def forward(self, seq, msa, msa_mask = None, **kwargs):
        ...
        return self.kirai(seq, msa, seq_embed = seq_embed, msa_embed = msa_embed, msa_mask = msa_mask, **kwargs)
...
class MSAEmbedWrapper(nn.Module):
    def __init__(self, *, kirai):
        ...
        self.project_embed = nn.Linear(MSA_EMBED_DIM, kirai.dim) if MSA_EMBED_DIM != kirai.dim else nn.Identity()
...
class ESMEmbedWrapper(nn.Module):
    def __init__(self, *, kirai):
        ...
        self.model = model
        self.batch_converter = batch_converter
        self.project_embed = nn.Linear(ESM_EMBED_DIM, kirai.dim) if ESM_EMBED_DIM != kirai.dim else nn.Identity()
    ...
    def forward(self, seq, msa=None, **kwargs):
        ...
        return self.kirai(seq, msa, seq_embed = seq_embeds, msa_embed = msa_embeds, **kwargs)
